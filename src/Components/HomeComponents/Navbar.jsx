        ...
        <div className="nav flex justify-center items-center gap-x-12">
          {
            ['Home', 'About', Logo, 'Gallery', 'Contact'].map((item, idx) => {
              return idx == 2 ? <img key={idx} src={item} className='w-40 mb-2'/> : (<span key={idx} className='cursor-pointer hover:-translate-y-2 duration-300 ease-in py-1'>{item}</span>)
            })
          }
        </div>
        <div className="icons flex gap-x-2">
          {
            [GoPerson, GoHeart, CgShoppingCart].map((Icon, idx) => (
              <span key={idx} className='text-3xl cursor-pointer hover:opacity-50'>
                <Icon/>
              </span>
            ))
          }
